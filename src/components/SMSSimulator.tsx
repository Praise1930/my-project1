      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          ...
          .sms-simulator-drawer {
            ...
            height: ${isMaximized ? 'calc(100vh - 120px)' : '480px'} !important;
            top: ${yPos === null ? '120px' : `${yPos > window.innerHeight / 2 ? yPos - 490 : yPos + 50}px`} !important;
            bottom: auto !important;
            right: 16px !important;
          }
        }
      ...
      {isOpen && (
        <div 
          ...
          style={{
            ...
            top: yPos === null ? undefined : `${yPos > window.innerHeight / 2 ? (isMaximized && isDesktop ? yPos - 630 : yPos - 570) : yPos + 50}px`,
            right: '24px',
            zIndex: 99999,
            width: isMaximized && isDesktop ? '840px' : '420px',
            height: isMaximized && isDesktop ? '600px' : '560px',
        ...
        >
          ...
          {/* Navigation Tabs (Hidden in maximized desktop view) */}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column' }}>
            {isMaximized && isDesktop ? (
              /* Side-by-Side Double Pane Layout for Maximized Desktop View */
              <div style={{ display: 'flex', gap: '24px', height: '100%', alignItems: 'stretch' }}>
                <div style={{ flex: 1.2, display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(255, 255, 255, 0.08)', paddingRight: '24px' }}>
                  <h5 style={{ margin: '0 0 10px', fontSize: '0.85rem', fontWeight: 800, color: '#fb7185' }}>✉️ SMS Gateway Logs</h5>
                  {renderSmsLogs()}
                </div>
                <div style={{ flex: 0.8, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <h5 style={{ margin: '0 0 10px', fontSize: '0.85rem', fontWeight: 800, color: '#fb7185', textAlign: 'center' }}>📳 Dial USSD (*270#)</h5>
                  {renderUssdSimulator()}
                </div>
              </div>
            ) : (
              /* Normal Tabbed Layout for Default Desktop / Mobile View */
              <>
                {activeTab === 'sms' && renderSmsLogs()}
                {activeTab === 'ussd' && renderUssdSimulator()}
              </>
            )}
          </div>
        </div>
